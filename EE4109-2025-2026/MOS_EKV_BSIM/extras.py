...
def printOPinfo(device, biasPar, ID, VG, VD, VS, VB, f):
    ...
    print('\nDerived parameters:\n')
    print('fT    :', sp.N(device.params['ggs']/(2*np.pi*device.params['cgg']),4))
    print('mu    :', sp.N(device.params['ggs']/device.params['gds'], 4))
    print('gm/Id :', sp.N(device.params['ggs']/device.params['i(ids)'], 4))

def plotOpinfo(device, biasPar, ID, VG, VD, VS, VB, f, step, EKVlib=None):
    if device.dev == "pch":
        ID = -ID
        VGS = VS-VG
    elif device.dev == 'nch':
    ...
    derivedParams['gm/Id\t'] = device.params['ggs']/device.params['i(ids)']
    ...
    if EKVlib != None:
        ...
        cirText = "EKV model\n"
    if device.dev == "nch":
        cirText += "X1 D G S 0 CMOS18N W={W} L={L} ID={I_D} \n.param I_D=0 W=%s L=%s\n.end\n"%(device.W * device.M, device.L)
    elif device.dev == "pch":
        cirText += "X1 D G S 0 CMOS18P W={W} L={L} ID={-I_D} \n.param I_D=0 W=%s L=%s\n.end\n"%(device.W * device.M, device.L)
    f = open('cir/MOS.cir', 'w')
    f.write(cirText)
    f.close()
    cir = makeCircuit('cir/MOS.cir', imgWidth=None)
    cir.delPar('I_D')
    I_D = sp.Symbol('I_D')
    IDS = device.params['i(ids)']
    if device.dev == "nch":
        VGS               = sp.lambdify(I_D, cir.getParValue('V_GS_X1'))
    ...
    fig_vgs = plot('fig_vgs', '$I_{ds}$ versus $V_{gs}$', 'semilogy', plotDict_vgs, yScale='u', xName='$V_{gs}$', yName='$I_{ds}$', xUnits='V', yUnits='A', show=True)

    GM                = sp.lambdify(I_D, cir.getParValue('g_m_X1'))
    gmTraceEKV        = trace([IDS, GM(IDS)])
    gmTraceEKV.label  = 'EKV'
    gmTraceBSIM       = trace([IDS, device.params['ggs']])
    ...
    FT                = sp.lambdify(I_D, cir.getParValue('f_T_X1'))
    ...
    GB                = sp.lambdify(I_D, cir.getParValue('g_b_X1'))
    gbTraceEKV        = trace([IDS, GB(IDS)])
    gbTraceEKV.label  = 'EKV'
    gbTraceBSIM       = trace([IDS, device.params['gbs']])
    ...
    GO                = sp.lambdify(I_D, cir.getParValue('g_o_X1'))
    goTraceEKV        = trace([IDS, GO(IDS)])
    goTraceEKV.label  = 'EKV'
    goTraceBSIM       = trace([IDS, device.params['gds']])
...
def plotSvinoise(device, ID, VD, VS, VB, fmin, fmax, numDec, EKVlib='SLiCAP.lib'):
    ...
    cirText += ".lib %s\n"%(EKVlib)
    if device.dev == "nch":
        cirText += "X1 in 0 out  MN18_noise W={W} L={L} ID={I_D}\n"
    elif device.dev == "pch":
        cirText += "X1 in 0 out  MP18_noise W={W} L={L} ID={I_D}\n"
    cirText += "V1 in 0 1\n"
    cirText += ".end\n"
    ...
    cir = makeCircuit("cir/EKVmosNoise.cir", imgWidth=None)
    cir.defPar('W', device.W * device.M)
    cir.defPar('L', device.L)
    cir.defPar('I_D', ID)
    result = doNoise(cir, pardefs='circuit', source='V1', detector='V_out', numeric=True)
    result.detLabel = "-EKV"
    ...
    figNoise.plot()
    FT = cir.getParValue("f_T_X1")
    FL = cir.getParValue("f_ell_X1")
    print("f_T     =", sp.N(FT,3))
    print("f_L     =", sp.N(FL,3))
