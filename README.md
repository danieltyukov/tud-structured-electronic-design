...
The interesting part is not the final schematic, it is **how the design is reached**. This repo
...
| Audio band | 600 Hz to 6 kHz | Hearing-loop standard passband |
...
| Compensation | one resistor `R_phz ≈ 0.42 kΩ` | A **phantom zero** pulls a high-Q pole pair from Q ≈ 5.4 to 0.70 (maximally flat) |
...
below 0 dB** across the band (black curve), so feedback cannot enforce the transfer and accuracy
...
**left half-plane (stable)**, but one pair near 51 MHz has **Q ≈ 5.4**, so it peaks.
...
> **SLiCAP returns equations; ngspice/LTspice return numbers, and every symbolic result is verified numerically.**
...
Eight notebooks form a chain, each reading CSV artifacts written by the previous ones:
...
# plus ngspice 42 and KiCad 9 from your package manager

# interactive: study each notebook in order
cd Notebooks
jupyter notebook            # Run-All each notebook 1 to 8, answering the input() prompts

# non-interactive: run all 8 in order (auto-answers the prompts)
python tools/run_hlr_notebooks.py
```

Outputs land under `Notebooks/`: design data (`csv/`), schematic and plot SVGs (`img/`), and HTML
